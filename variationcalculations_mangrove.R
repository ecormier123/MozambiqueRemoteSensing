...
names(allrasters) = paste("Y",substr((files),1,4),sep=".")
...
sum.ras<-apply(count.ras[2:3,], 2, sum)
...
plot(y=plot.data,  x=substr(colnames(count.ras),3,6), type="b", ylim=c(0,100))


plot.data2<-round((count.ras[3,]/sum.ras)*100,2)
points(y=plot.data2, x=substr(colnames(count.ras),3,6), type="b", col="red")

count.row<-apply(df.ras, 1, table, simplify = F)

emptydataframe<-matrix(NA, ncol=3, (nrow=dim(df.ras)[1]))

colnames(emptydataframe)<-c("x0", "x1", "x2")
